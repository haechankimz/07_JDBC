...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// Class.froName("클래스명") : 해당 클래스를 읽어 메모리에 적재
			...
			String port = ":1521"; // DB서버 컴퓨터에 DB 프로그램 연결번호
			...
			// ***** JDBC 에서 SQL 작성 시 ;을 작성하면 안된다 *****
			// -> "SQL 명령어가 올바르게 종료되지 않았습니다"
			
			
			/* 4. Statement 객체 생성 */
			stmt = conn.createStatement();
			// Connection에 연결된 DB에 SQL 전달하고 결과를 받아오는
			// Statement 객체 생성
			...
			while(rs.next()) {
				...
				// [java]	[db]
				//String	CHARm, VARCHAR2
				// int,long  NUMBER(벙수만 저장된 컬럼_
